...
#pragma warning(pop) 
BOOL CWndShadow::m_bForceEnable=FALSE;
...
void CWndShadow::Create(HWND hParentWnd,BOOL bForceEnable)
{
	m_bForceEnable = bForceEnable;
...
void CWndShadow::Show(HWND hParentWnd)
{
	...
	if((m_Status & SS_ENABLED) && !(m_Status & SS_DISABLEDBYAERO))	// Enabled
	{
		...
		DWORD lParentStyle = (DWORD)GetWindowLong(hParentWnd, GWL_STYLE);
